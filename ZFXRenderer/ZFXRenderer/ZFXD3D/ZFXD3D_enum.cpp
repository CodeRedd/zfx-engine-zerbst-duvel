...
//Found copy of enum code on Google Code here: https://code.google.com/p/3d-zfxengine/source/browse/ZFXEngine/ZFXD3D/ZFXD3D_enum.cpp?r=3&spec=svn3
...
void ZFXD3DEnum::ChangedAdapterFmt(void) {
	...
	if (!bWindowed) {
		...
		for (UINT m = 0; m<pA->nNumModes; m++) {
			dpsmd = &pA->d3ddspmd[m];
			swprintf(buffer, 200, L"%d x %d", dpsmd->Width, dpsmd->Height);
