...
HRESULT ZFXNetwork::CreateDevice(void)
{
	...
	m_hDLL = LoadLibraryEx(L"ZFXWS.dll", NULL, 0);
	if (!m_hDLL)
	{
		MessageBox(NULL, L"Loading ZFXWS.dll from lib failed.", L"ZFXEngine - error", MB_OK | MB_ICONERROR);
		return E_FAIL;
	}
	...
	if (FAILED(hr))
	{
		MessageBox(NULL, L"CreateNetworkDevice() from lib failed.", L"ZFXEngine - error", MB_OK | MB_ICONERROR);
		m_pDevice = NULL;
		return E_FAIL;
	}
