...
HRESULT ZFXAudio::CreateDevice(void)
{
	...
	m_hDLL = LoadLibraryEx(L"ZFXDA.dll", NULL, 0);
	if (!m_hDLL) 
	{
		MessageBox(NULL,L"Loading ZFXDA.dll from lib failed.", L"ZFXEngine - error", MB_OK | MB_ICONERROR);
		return E_FAIL;
	}
	...
	if (FAILED(hr))
	{
		MessageBox(NULL, L"CreateAudioDevice() from lib failed.", L"ZFXEngine - error", MB_OK | MB_ICONERROR);
		m_pDevice = NULL;
		return E_FAIL;
	}
